...
<body class="h-screen justify-between">
  <header>
    ...
    <div id="modalEl" tabindex="-1" aria-hidden="true" class="fixed left-0 right-0 top-0 z-50 hidden h-[calc(100%-1rem)] max-h-full w-full overflow-y-auto overflow-x-hidden p-4 md:inset-0 bg-white-800">
      <form class="bg-white p-6" action="/post/create" method="post" enctype="multipart/form-data">
        @csrf
        <div class="space-y-6">
          <div class="">
            ...
            <div class="mt-5 grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-6">
              <div class="sm:col-span-4">
                ...
                <div class="mt-2">
                  <div class="flex rounded-md shadow-sm ring-1 ring-inset ring-gray-300 focus-within:ring-2 focus-within:ring-inset focus-within:ring-indigo-600 sm:max-w-md">
                    <input required type="text" name="title" id="title" class="block flex-1 border-0 bg-transparent py-1.5 pl-1 text-gray-900 placeholder:text-gray-400 focus:ring-0 sm:text-sm sm:leading-6">
                  </div>
                </div>
              </div>
              ...
              <div class="col-span-full">
                ...
                <div class="mt-1">
                  <textarea required id="description" name="description" rows="2" max="64" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"></textarea>
                </div>
              ...
              <div class="col-span-full">
                ...
                <div class="mt-1">
                  <textarea required id="content" name="content" rows="4" max="255" class="block w-full rounded-md border-0 py-1.5 text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 placeholder:text-gray-400 focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6"></textarea>
                </div>
              </div>
              <div class="col-span-full">
                <label for="picture" class="block text-sm font-medium leading-6 text-gray-900">Picture</label>
                <div class="mt-2 flex justify-center rounded-lg border border-dashed border-gray-900/25 px-6">
                  <div class="text-center">
                    ...
                    <div class="mt-1 flex text-sm leading-6 text-gray-600">
                      <label for="file-upload" class="relative cursor-pointer rounded-md bg-white font-semibold text-primary-600 focus-within:outline-none focus-within:ring-2  focus-within:ring-offset-2">
                        <span>Upload a file</span>
                        <input required id="file-upload" name="image" type="file" class="sr-only">
                      </label>
                ...
                <div class="grid grid-cols-2 mt-3">
                  ...
                  @foreach($categories->unique('id') as $category)
                    <div class="flex items-center gap-x-3">
                      <input id="{{ $category['id'] }} " value="{{ $category['id'] }}" @if($category['name']=='Divers') checked @endif name="category_id" type="radio" class="h-4 w-4 border-gray-300 text-indigo-600 focus:ring-indigo-600">
        ...
        <div class="mt-6 flex items-center justify-end gap-x-6">
          ...
          <button type="submit" class="rounded-md bg-black px-3 py-2 text-sm font-semibold text-white shadow-sm  focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-2 ">Save</button>
        </div>
      </form>
    </div>
  </header>
